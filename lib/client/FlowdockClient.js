...
class FlowdockClient {
   constructor({ session }) {
      ...
      if (typeof session === 'string') {
         ...
         throw new TypeError('Expected type string or object in session constructor parameter');
      }
   ...
   async request({ type = 'get', path, data }) {
      ...
      return new Promise((resolve, reject) => {
         this.session[type](path, (err, resData, rawRes) => {
            if (err) {
               reject(err);
               return;
            }
            ...
            if (rawRes && rawRes.headers && rawRes.headers['Flowdock-User']
               ...
               this._selfIds.add(parseInt(rawRes.headers['Flowdock-User']));
            }
   ...
   async retrieveFlows() {
      ...
      for (const flowData of flowDataList) {
         flowStructureList.push(new Flow(this, flowData));
      }

      return flowStructureList;
   }
   ...
   async retrieveOrganizations() {
      ...
      for (const organizationData of organizationDataList) {
         organizationStructures.push(new Organization(this, organizationData));
      }
