...
class FlowdockClient extends EventEmitter  {
   ...
   async retrieveUsers() {
      ...
      for (const organization of this.organizations.values()) {
         let userDataList;
         try {
            userDataList = this.request({ path: `/organizations/${organization.id}/users` });
         } catch (e) {
            throw e;
         }
   ...
   _startFlowUpdater() {
      ...
      this._flowUpdaterInterval = setInterval(() => {
   ...
   async init() {
      ...
      // Now that all users are populated, go back
      // and set all self users.
      for (const id of Array.from(this._selfIds)) {
         const user = this.users.get(id);

         this.self.set(id, user);

         // Set the self for the user's organization
