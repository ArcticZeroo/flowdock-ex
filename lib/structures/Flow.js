...
class Flow extends Structure {
   ...
   setup(data) {
      ...
      data.organization = this.client.organizations.get(data.organization.id);
      ...
      data = ObjectUtil.convertProperties(data, true);
   ...
   thread(threadId, message, tags) {
      return this.client.session.threadMessage(this.id, threadId, message, tags);
   }

   comment(parentId, comment, tags) {
      return this.client.session.comment(this.id, parentId, comment, tags);
   }
